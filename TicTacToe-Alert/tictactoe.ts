...
import {
    ...
    assert,
    ByteString,
    SigHash, PubKey, FixedArray, fill, Sig, hash160, toByteString, Utils, sha256, bsv, buildOpreturnScript, toHex
} from 'scrypt-ts'
...
export class TicTacToe extends SmartContract {
    ...
    public alert(sig: Sig, outputMsg: ByteString) {    

        assert(this.checkSig(sig, this.bob), `checkSig failed, pubkey: ${this.bob}`);

        let outputs = toByteString('');
        //let outMsg = toByteString('');

        outputs = this.buildStateOutput(this.ctx.utxo.value);
        outputs += Utils.buildPublicKeyHashOutput(hash160(this.alice), 1n);
        outputs += Utils.buildPublicKeyHashOutput(hash160(this.bob), 1n);
        //outMsg = Utils.buildOpreturnScript(msg)

        //outputs += toByteString('0000000000000000') + (Utils.buildOpreturnScript(msg));
        //outputs += toByteString('0000000000000000' ) + outMsg;
        outputs += outputMsg

        //toHex((unsignedTx.outputs[3]).toBufferWriter().toBuffer())
        
        /*
        outputs += toByteString(toHex(((new bsv.Transaction.Output({
            script: buildOpreturnScript(msg),
            satoshis: 0
        })).toBufferWriter().toBuffer())))
        */
        
        //console.log('MSG Contract: ', msg)
