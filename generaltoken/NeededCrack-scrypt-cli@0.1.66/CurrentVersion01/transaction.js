...
var newIdx = 0
var pvTXIDs = []  //TXID que serao usados pelos novos UTXOs
var pvNewIdx = [] //Indices dos Outputs a serem utilizados
var scriptSHA256 = [] //Hashes dos scrips dos UTXOs
var takenUTXO = [] //Verifica se os UTXOs da lista ja foram utilizados
...
function Transaction (serialized) {
  ...
  //this.DERSECval = 'abc'
  this.DERSECval = []				//acrescetar
  this.nDERSig = 0				//acrescetar
...
//Jesus is the Lord
Transaction.FEE_PER_KB = 1 //new Fee
...
//Jesus is the Lord
Transaction.prototype.kDet = function (kDet) {					//acrescentar
  //this.pvOutIx = idx
  var sig = ECDSA.kDet(kDet)
  //ECDSA.verify(hashbuf, signature, publicKey, 'little')
  //return this.DERSECval
}

//Jesus is the Lord
Transaction.prototype.invN = function (kN) {					//acrescentar
  //this.pvOutIx = idx
  var invK = ECDSA.invN(kN)
...
//Jesus is the Lord
Transaction.prototype.preimageExt = function () {					//acrescentar
  //console.log("preImage EXT 2 TX:", ECDSA.preimageExtOut())
  return ECDSA.preimageExtOut()
}

//Jesus is the Lord
Transaction.prototype.sigR = function () {					//acrescentar
  //console.log("preImage EXT 2 TX:", ECDSA.preimageExtOut())
  return ECDSA.sigRout()
}

//Jesus is the Lord
Transaction.prototype.sigS = function () {					//acrescentar
  //console.log("preImage EXT 2 TX:", ECDSA.preimageExtOut())
  return ECDSA.sigSout()
}

//Jesus is the Lord
Transaction.prototype.ditacticRST = function () {					//acrescentar
  //this.pvOutIx = idx
  ECDSA.ditacticRST();
...
//Jesus is the Lord
Transaction.prototype.ecdsaForge = function (aIn, bIn, xQA, isOddY) {					//acrescentar
  //console.log("preImage EXT 2 TX:", ECDSA.preimageExtOut())
  return ECDSA.ecdsaForge(aIn, bIn, xQA, isOddY)
}

//Jesus is the Lord
Transaction.prototype.ecdsaAlt = function (e, k, pkv) {					//acrescentar
  //console.log("preImage EXT 2 TX:", ECDSA.preimageExtOut())
  return ECDSA.ecdsaAlt(e, k, pkv)
  //return ECDSA.ecdsaForge(aIn, bIn, xQA, isOddY)
}

//Jesus is the Lord
Transaction.prototype.pbkeyFromECDSA = function (zIn, sIn, rIn) {					//acrescentar
  //console.log("preImage EXT 2 TX:", ECDSA.preimageExtOut())
  return ECDSA.pbkeyFromECDSA(zIn, sIn, rIn)
}
...
Transaction.prototype._estimateFee = function () {
  var estimatedSize = this._estimateSize()
  //console.log('estimate size', 2*estimatedSize)
  console.log('estimate size', estimatedSize)
  console.log('f/kb', (Transaction.FEE_PER_KB))
  console.log('f/kb', (Transaction.FEE_PER_KB))
  //this._feePerKb = 101
  //console.log('f/kb', (this._feePerKb || Transaction.FEE_PER_KB))
//  console.log('fee', Math.ceil((1 + 1/2)*estimatedSize / 1000 * (this._feePerKb || Transaction.FEE_PER_KB)))
  //return Math.ceil((1 + 1/2)*estimatedSize / 1000 * (this._feePerKb || Transaction.FEE_PER_KB))
  return Math.ceil(estimatedSize / 1000 * (this._feePerKb || Transaction.FEE_PER_KB))
}
...
Transaction.prototype.applySignature = function (signature) {					//achar
  //  console.log('\n xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
  //  console.log('sign 2 apply b', scryptlib_1.toHex(this))
  //  console.log('sign 2 DER SIG', scryptlib_1.toHex(signature.signature.toDER()))
    //console.log('sign 2 apply b2', scryptlib_1.toHex(signature.publicKey[0].sec))
    this.inputs[signature.inputIndex].addSignature(this, signature)
    //this.inputs[signature.inputIndex].addSignature(this, sfasdf)
    //this.inputs[signature.inputIndex].
  //  console.log('sign 2 apply a', scryptlib_1.toHex(this))
    this.signature = signature									//acrescentar
  
    //console.log('sign XXX', signature)
  
  //  console.log('sign 2 DER', scryptlib_1.toHex((this.signature.signature).toDER()))
  //  console.log('sign 2 SEC', scryptlib_1.toHex((this.signature.publicKey).toDER()))
    let DER = scryptlib_1.toHex((this.signature.signature).toDER()) + "41"			//acrescentar
    let sizeDER = (DER.length/2).toString(16)							//acrescentar
    let SEC = scryptlib_1.toHex((this.signature.publicKey).toDER())				//acrescentar
    let sizeSEC = (SEC.length/2).toString(16)							//acrescentar
    let sizeScript = ((sizeDER + DER + sizeSEC + SEC).length/2).toString(16)			//acrescentar
  
  
    if(sizeDER.length % 2 === 1)  sizeDER = '0'+ sizeDER						//acrescentar
    if(sizeSEC.length % 2 === 1)  sizeSEC = '0'+ sizeSEC						//acrescentar
    if(sizeScript.length % 2 === 1)  sizeScript = '0'+ sizeScript 				//acrescentar
  
  
  
    this.DERSECval[this.nDERSig] = sizeScript + sizeDER + DER + sizeSEC + SEC			//acrescentar
    this.nDERSig ++;  										//acrescentar
    //console.log('sign 2 fullySig', ((this.signature.isFullySigned)))
    //console.log('sign 2 fullySig', ((this.si)))
...
Transaction.prototype.DERSEC = function () {					//acrescentar
  return this.DERSECval
}
...
Transaction.prototype.newIndex = function (idx) {					//acrescentar
  //this.pvOutIx = idx
  newIdx = idx
  //return this.DERSECval
}
...
Transaction.prototype.pvTxIdx = function (txid, idx, scriptSHA256ext) {					//acrescentar
  var i = 0;
  while(typeof pvNewIdx[i] !== "undefined")
  {
      i++;
  }
  pvTXIDs[i] = txid;
  pvNewIdx[i] = idx
  scriptSHA256[i] = scriptSHA256ext
  takenUTXO[i] = false
}
...
//  console.log('Number of PV TXs: ', pvNewIdx.length)
  //console.log('Number of PV TXs: ', pvNewIdx.outputs)
  outputIdx = newIdx

  for(var i=0; i< pvNewIdx.length; i++)
  {
  //  console.log('Before If: ', pvTXIDs[i])
  //  console.log('Number of UTXOs: ', prevTx.outputs.length)
    var breakflag = false
    //console.log('Script: ', scryptlib_1.sha256(scryptlib_1.toHex(prevTx.outputs[0].script.buffer)))

    for(var j=0; j < prevTx.outputs.length; j++ )
    {
      if(prevTx.id === pvTXIDs[i] && 
        scryptlib_1.sha256(scryptlib_1.toHex(prevTx.outputs[j].script.buffer)) === scriptSHA256[i] && 
        !takenUTXO[i] )
      {
        //não pode ser comparado antes, pois podem existir mais UTXO registrados do que existem na TX
        //if(scryptlib_1.sha256(scryptlib_1.toHex(prevTx.outputs[j].script.buffer)) === scriptSHA256[i] && !takenUTXO[i] )
        //{ }
        takenUTXO[i] = true
      //  console.log('Script Sequency: ', scryptlib_1.sha256(scryptlib_1.toHex(prevTx.outputs[j].script.buffer)))
      //  console.log('Position j: ', j)
      //  console.log('Position i: ', i)
      //  console.log('Position index: ', pvNewIdx[i])
        outputIdx = pvNewIdx[i]
      //  console.log('Inside If: ', pvTXIDs[i])
        breakflag = true
        break;
      }
    }
  ...
  //outputIdx = newIdx
  //console.log('Previous Tx prevTx: ', prevTx.id)
  //console.log('Previous Tx prevTx: ', prevTx.outputs)
//  console.log('TX outputIndex: ', outputIndex)
//  console.log('TX outputIndex: ', outputIdx)
