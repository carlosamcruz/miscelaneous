...
ECDSA.ecdsaAlt = function (e, k, pvk) {
  var N = Point.getN()
  var G = Point.getG()

  var z1 = new BN(BigInt("0x" + e)) // PVT Key

  var k1 = new BN(BigInt("0x" + k)) // k

  var dA = new BN(BigInt("0x" + pvk)) // k

  var kG, kInv, r1, rdA, zrdA, s1, s2, QA

  kG = G.mul(k1)

  kInv = k1.invm(N)

  r1 = new BN(1).mul(kG.x.umod(N))

  rdA = r1.mul(dA).umod(N)
  zrdA = z1.add(rdA).umod(N)

  s1 = kInv.mul(zrdA).umod(N)
  s2 = N.sub(s1)

  QA = G.mul(dA)

  var z32 = Buffer.from(z1.toBuffer()).reverse().toString('hex')
  ...
  var sigVer1 = ECDSA.verify(Buffer.from(new Uint8Array(bytes)), new Signature(r1, s1), new PublicKey(QA), 'little')
  var sigVer2 = ECDSA.verify(Buffer.from(new Uint8Array(bytes)), new Signature(r1, s2), new PublicKey(QA), 'little')

  var result;

  if(sigVer1 && sigVer2)
    //result = [z.toString('hex'), r.toString('hex'),s.toString('hex'), sinv.toString('hex')]             
    result = [z1.toString('hex'), r1.toString('hex'),s1.toString('hex'), s2.toString('hex')]             
  else
    result = ['Failed', 'Failed','Failed', 'Failed']                
